...
- **Represent biomedical entities clearly:** LifeSphere separates stable biological entities such as `Gene`, `Protein`, `Pathway`, `Variant`, `CpGSite`, `CellType`, and `Disease` from sample-specific observations such as `ExpressionObservation`, `VariantObservation`, `MethylationObservation`, `ProteinObservation`, and `PhenotypeObservation`.
...
The molecular observation backbone links a `Sample` to its sample-specific observations, and links each observation outward to the stable biological entity it measures:
...
The proteomic path extends this backbone through the stable `Protein` entity, which mirrors `Gene` as a reference node while sample-specific abundance evidence lives on `ProteinObservation`. `Protein` and `Gene` both connect outward to `Pathway` for curated biological-context traversal:

```cypher
(:Gene)-[:ENCODES]->(:Protein)
(:Protein)-[:PARTICIPATES_IN_PATHWAY]->(:Pathway)
...
`MAPS_TO_PROTEIN` is used when a source feature is an antibody-derived tag (ADT) or other protein-level feature, such as in CITE-seq panels, rather than a gene-level feature.
...
flowchart LR
  ...
  ProcessedDataOutput -->|CONTAINS_OBSERVATION| ProteinObservation
  ...
  VariantObservation -->|IMPACTS_GENE| Gene
  ProteinObservation -->|MEASURES_PROTEIN| Protein[Protein]

  %% Proteomics and pathways
  Gene -->|ENCODES| Protein
  Protein -->|PARTICIPATES_IN_PATHWAY| Pathway[Pathway]
  Gene -->|PARTICIPATES_IN_PATHWAY| Pathway
  Pathway -->|PART_OF_PATHWAY| Pathway
  ...
  Publication -->|EVIDENCES| Protein
  Publication -->|EVIDENCES| Pathway
...
| `Protein`                | Stable protein reference entity encoded by a gene, preferably keyed by UniProt or organism-appropriate identifiers. Mirrors `Gene` as a stable biological entity; sample-specific abundance evidence is represented through `ProteinObservation`. | `proteinId` |
| `Pathway`                | Stable, curated biological pathway, process, or gene/protein set, preferably ontology- or database-backed (e.g. Reactome, KEGG, GO Biological Process). Represents reference-level pathway membership and hierarchy, not sample-specific pathway activity or enrichment scoring. | `pathwayId` |
...
| `ProteinObservation`     | Sample-specific or source-provided protein abundance or post-translational-modification measurement linked to a stable protein entity.                                                                                                                                                                                                                                        | `proteinObservationId`     |
...
| `(:ProcessedDataOutput)-[:CONTAINS_OBSERVATION]->(:ProteinObservation)` | `CONTAINS_OBSERVATION` | `ProcessedDataOutput` | `ProteinObservation` | Explicit linkage from matrix to nodes. |
...
| `(:Assay)-[:GENERATED_OBSERVATION]->(:ProteinObservation)` | `GENERATED_OBSERVATION` | `Assay` | `ProteinObservation` | Direct link from physical run to biological finding. |
...
| `(:Sample)-[:HAS_PROTEIN_OBSERVATION]->(:ProteinObservation)` | `HAS_PROTEIN_OBSERVATION` | `Sample` | `ProteinObservation` | Links biological sample to its molecular reading. |
| `(:ProteinObservation)-[:MEASURES_PROTEIN]->(:Protein)` | `MEASURES_PROTEIN` | `ProteinObservation` | `Protein` | Maps finding to a canonical protein. |
...
| `(:Publication)-[:EVIDENCES]->(:Protein)` | `EVIDENCES` | `Publication` | `Protein` | General NLP mapping. Links source text to protein-level evidence. |
| `(:Publication)-[:EVIDENCES]->(:Pathway)` | `EVIDENCES` | `Publication` | `Pathway` | General NLP mapping. Links source text to pathway-level evidence. |
...
| `(:Publication)-[:EVIDENCES]->(:ProteinObservation)` | `EVIDENCES` | `Publication` | `ProteinObservation` | Links source text to a protein observation it supports. |
...
| `(:Feature)-[:MAPS_TO_PROTEIN]->(:Protein)` | `MAPS_TO_PROTEIN` | `Feature` | `Protein` | Maps a source protein-level feature, such as a CITE-seq antibody-derived tag (ADT), to a canonical protein. |
...
### 5.10 Proteomic and Pathway Relationships

| Relationship Pattern | Relationship Type | source_node | target_node | Purpose |
| -------------------- | ----------------- | ----------- | ----------- | ------- |
| `(:Gene)-[:ENCODES]->(:Protein)` | `ENCODES` | `Gene` | `Protein` | Links the stable gene entity to the stable protein entity it encodes. |
| `(:Protein)-[:PARTICIPATES_IN_PATHWAY]->(:Pathway)` | `PARTICIPATES_IN_PATHWAY` | `Protein` | `Pathway` | Reference-level membership of a protein in a curated biological pathway. |
| `(:Gene)-[:PARTICIPATES_IN_PATHWAY]->(:Pathway)` | `PARTICIPATES_IN_PATHWAY` | `Gene` | `Pathway` | Reference-level membership of a gene in a curated pathway or gene set, for sources (e.g. many KEGG/MSigDB gene sets) that define membership at the gene level rather than the protein level. |
| `(:Pathway)-[:PART_OF_PATHWAY]->(:Pathway)` | `PART_OF_PATHWAY` | `Pathway` | `Pathway` | Hierarchical containment between a lower-level pathway/sub-process and its parent pathway, as provided by hierarchical sources such as Reactome. |

Note on directionality: `PARTICIPATES_IN_PATHWAY` is used from both `Protein` and `Gene` because pathway databases provide membership at different molecular levels. Prefer the protein-level relationship when the source pathway definition is protein/complex-based (e.g. Reactome reactions), and the gene-level relationship when the source defines membership by gene identifier alone (e.g. many KEGG or MSigDB gene sets). Do not infer one from the other; populate only the relationship that is directly supported by the source pathway data, consistent with the anti-hardcoding principle in Section 2.5.
...
### 6.9 Proteomic and Pathway-Specific Design

Proteomic data follow the same stable-entity-versus-observation split used for transcriptomic and genomic data, and pathway data are represented as reference-level biological context attached to `Gene` and `Protein`.

- `Protein` represents the stable protein reference entity, preferably keyed by UniProt accession. It mirrors `Gene` as a stable biological entity and is linked back to its coding gene through `(:Gene)-[:ENCODES]->(:Protein)`.
- `ProteinObservation` represents a sample-specific or source-provided protein abundance measurement, analogous to `ExpressionObservation`. It supports both global proteomics (e.g. mass-spectrometry-based total protein abundance) and post-translational-modification (PTM) proteomics (e.g. phosphoproteomics), distinguished using `modificationType` rather than separate node labels.
- Antibody-based proteomic assays, such as reverse-phase protein arrays (RPPA) or targeted panels (e.g. Olink), are represented using the same `ProteinObservation` node with `quantificationMethod` and `platform` recording the assay technology.
- Dense proteomic matrices, such as mass-spectrometry intensity matrices, should remain outside Neo4j and be represented through `ProcessedDataOutput`, consistent with the external matrix strategy in Section 2.6.
- LifeSphere should not imply current-stage differential abundance analysis, enrichment, or pathway-activity scoring unless those outputs are explicitly source-provided or marked as future-use, consistent with Section 1.2.
- `Pathway` represents a stable, curated pathway, biological process, or gene/protein set (e.g. Reactome, KEGG, GO Biological Process). It stores reference-level pathway identity and hierarchy only; it is not a sample-specific pathway-activity or enrichment-score node.
- Pathway membership is represented explicitly at the molecular level that the source database supports, using `(:Protein)-[:PARTICIPATES_IN_PATHWAY]->(:Pathway)` and/or `(:Gene)-[:PARTICIPATES_IN_PATHWAY]->(:Pathway)`, as described in Section 5.10.
- Pathway hierarchy (e.g. a Reactome sub-pathway nested within a parent pathway) is represented using `(:Pathway)-[:PART_OF_PATHWAY]->(:Pathway)`, not by duplicating parent-level properties onto child pathways.
- Single-cell proteomic or multimodal panels, such as CITE-seq antibody-derived tags, map source-level antibody features to `Protein` through `(:Feature)-[:MAPS_TO_PROTEIN]->(:Protein)`, mirroring the existing `(:Feature)-[:MAPS_TO_GENE]->(:Gene)` pattern described in Section 6.6.3.
- Future pathway-activity, enrichment, or pathway-level statistical scoring (e.g. GSEA, ssGSEA, pathway-level differential activity) is out of scope for the current active schema and should be documented as future-use, consistent with Section 1.2, rather than represented as a raw measurement on `Pathway` or `ProteinObservation`.
...
### 7.7 Proteomic and Pathway Node Properties

#### `Protein`

| Property | is_key | Data Type | Description | Example | Source / Origin |
| -------- | ------ | --------- | ----------- | ------- | --------------- |
| `proteinId` | Yes | String | Primary Key; stable protein identifier (UniProt-based; not keyed by symbol). | `uniprot:P04637` | Reference DB |
| `uniprotId` | No | String | UniProt accession. | `P04637` | UniProt |
| `symbol` | No | String | Approved protein/gene symbol associated with the protein. | `TP53` | HGNC / UniProt |
| `name` | No | String | Full protein name. | `Cellular tumor antigen p53` | UniProt |
| `organismTaxonId` | No | String | NCBI Taxonomy ID of the organism. | `9606` | NCBI Taxonomy |
| `geneId` | No | String | ID of the encoding Gene; represents `(:Gene)-[:ENCODES]->(:Protein)` | `ENSG00000141510` | Derived / Ensembl |
| `geneSymbol` | No | String | Mirror of the encoding gene's symbol, for convenience lookups. | `TP53` | Derived / HGNC |
| `aliases` | No | List | Alternative names / synonyms. | `["p53","LFS1"]` | Reference DB |
| `sequenceLength` | No | Integer | Length of the canonical protein sequence, in amino acids. | `393` | UniProt |
| `molecularWeightDa` | No | Float | Molecular weight of the canonical protein sequence. | `43653.0` | UniProt |
| `description` | No | String | Free-text description of the entity. | `Tumor suppressor; regulates cell cycle` | Reference DB |
...
| Property | is_key | Data Type | Description | Example | Source / Origin |
| -------- | ------ | --------- | ----------- | ------- | --------------- |
| `proteinObservationId` | Yes | String | Primary Key; identifies this protein measurement/result. | `obs_TCGA-BH_P04637_abs` | Derived |
| `observationType` | No | String | absolute_abundance \| differential_abundance \| ptm_abundance. | `absolute_abundance` | Pipeline config |
| `abundanceValue` | No | Float | Quantified protein abundance value. | `12.4` | Proteomics Matrix |
| `abundanceUnit` | No | String | Unit of the abundance value (e.g. intensity, NPX, log2_ratio, reverse-phase signal). | `log2_intensity` | Pipeline config |
| `abundanceStatus` | No | String | Categorical abundance status. | `detected` | Derived |
| `modificationType` | No | String | Post-translational modification measured, if any; `total_protein` for non-PTM abundance. | `phosphorylation` | Pipeline config |
| `modificationSite` | No | String | Residue/site of the modification, if applicable. | `S15` | Pipeline config / Annotation |
| `log2FoldChange` | No | Float | Differential log2 fold change. | `-1.20` | Derived |
| `pValue` | No | Float | Raw p-value (differential). | `0.004` | Derived |
| `adjustedPValue` | No | Float | Multiple-testing adjusted p-value. | `0.03` | Derived |
| `differentialAbundanceStatus` | No | String | Direction of differential abundance. | `down` | Derived |
| `quantificationMethod` | No | String | Quantification technology or approach (e.g. TMT, label-free, DIA, RPPA, Olink). | `TMT` | Pipeline config |
| `platform` | No | String | Instrument or panel platform used. | `Orbitrap Fusion Lumos` | Metadata / Config |
| `normalizationMethod` | No | String | Normalization applied. | `median_centering` | Pipeline config |
| `sourceDataset` | No | String | Originating dataset / cohort. | `CPTAC-BRCA` | Metadata |
| `sourceFile` | No | String | Originating source file. | `CPTAC-BRCA.protein_abundance.tsv` | Metadata |
| `pipelineVersion` | No | String | Pipeline version that produced the measurement. | `2.1.0` | Pipeline config |
| `sourcePublicationId` | No | String | Publication ID supporting this protein observation, if applicable. | `doi:10.1038/s41586-020-2725-7` | YAML/configuration / publication metadata / source annotation |
| `assayId` | No | String | ID of the Assay; represents `(:Assay)-[:GENERATED_OBSERVATION]->(:ProteinObservation)` |  | Derived |
| `sampleId` | No | String | ID of the Sample; represents `(:Sample)-[:HAS_PROTEIN_OBSERVATION]->(:ProteinObservation)` |  | Metadata |
| `contrastId` | No | String | Source-provided or future-use AnalysisContrast identifier |  | Derived |
| `proteinId` | No | String | ID of the Protein; represents `(:ProteinObservation)-[:MEASURES_PROTEIN]->(:Protein)` | `uniprot:P04637` | Derived |
| `outputId` | No | String | ID of the ProcessedDataOutput; represents `(:ProcessedDataOutput)-[:CONTAINS_OBSERVATION]->(:ProteinObservation)` | `out_proteinMatrix_CPTAC_BRCA` | Derived |
| `groupLabel` | No | String | reference or control label of differential_abundance | `tumor` | Pipeline config |
| `proteinSymbol` | No | String | Mirror of the protein/gene symbol. | `TP53` | Derived |
...
| Property | is_key | Data Type | Description | Example | Source / Origin |
| -------- | ------ | --------- | ----------- | ------- | --------------- |
| `pathwayId` | Yes | String | Primary Key; stable, database-qualified pathway identifier. | `reactome:R-HSA-109581` | Reactome / KEGG / WikiPathways / GO |
| `name` | No | String | Human-readable pathway name. | `Apoptosis` | Pathway database |
| `description` | No | String | Free-text description of the pathway. | `Programmed cell death mediated by caspase activation.` | Pathway database |
| `sourceDatabase` | No | String | Database or ontology of origin. | `Reactome` | Config |
| `pathwayCategory` | No | String | Broad category or top-level grouping of the pathway. | `Signal Transduction` | Pathway database |
| `organismTaxonId` | No | String | NCBI Taxonomy ID of the organism the pathway applies to. | `9606` | Pathway database |
| `pathwayVersion` | No | String | Release or version of the pathway database used. | `v86` | Config |
| `url` | No | String | Reference URL for the pathway record. | `reactome.org/content/detail/R-HSA-109581` | Pathway database |


Important modelling rule: `Pathway` stores curated, reference-level biological context only. Sample-specific pathway activity, enrichment scores, or pathway-level statistical results are out of current scope and should be documented as future-use, consistent with Section 1.2 and Section 6.9, rather than stored as properties on `Pathway`.
...
| `HAS_PROTEIN_OBSERVATION` | `(No properties)` | No | None | Pure structural link. | `N/A` |
...
| `MEASURES_PROTEIN` | `epitope` | No | String | Epitope or antibody-target region measured, where applicable (e.g. RPPA/Olink assays). | `phospho-S15` |
...
| `MAPS_TO_PROTEIN` | `mappingSource` | Yes | String | Source of feature-to-protein mapping. | `AnnData var.antibody_target` |
| `MAPS_TO_PROTEIN` | `panelName` | No | String | Antibody panel name or version, where applicable. | `TotalSeq-C Human Universal Cocktail` |
...
### 8.10 Proteomic and Pathway Relationship Attributes

| Relationship Type | Property | is_key | Data Type | Description | Example |
| ----------------- | -------- | ------ | --------- | ----------- | ------- |
| `ENCODES` | `(No properties)` | No | None | Pure structural link between a stable gene entity and the stable protein entity it encodes. | `N/A` |
| `PARTICIPATES_IN_PATHWAY` | `evidenceType` | No | String | Type of evidence supporting pathway membership. | `curated_annotation` |
| `PARTICIPATES_IN_PATHWAY` | `evidenceLevel` | No | String | Strength or reliability category of the supporting evidence. | `database_curated`, `experimentally_validated`, `predicted` |
| `PARTICIPATES_IN_PATHWAY` | `sourceDatabase` | No | String | Database supporting the pathway membership assertion. | `Reactome` |
| `PARTICIPATES_IN_PATHWAY` | `role` | No | String | Role of the gene/protein within the pathway, where available (e.g. catalyst, regulator, input, output). | `catalyst` |
| `PARTICIPATES_IN_PATHWAY` | `pathwayVersion` | No | String | Version of the pathway database used to assign membership. | `v86` |
| `PART_OF_PATHWAY` | `hierarchyLevel` | No | Integer | Depth/level of the child pathway within the parent pathway hierarchy, where available. | `2` |
| `PART_OF_PATHWAY` | `sourceDatabase` | No | String | Database supporting the pathway hierarchy relationship. | `Reactome` |
...
- **Primary keys:** Add uniqueness constraints for stable primary identifiers such as `sampleId`, `subjectId`, `geneId`, `proteinId`, `pathwayId`, `cellSetId`, `outputId`, and `methylationObservationId`.
- **Lookup fields:** Add indexes for high-frequency filters such as sample class, source dataset, assay type, gene symbol, protein symbol/UniProt ID, pathway source database, ontology ID, and observation mirror IDs.
...
### Example: find samples with protein abundance observations for a gene's encoded protein

```cypher
MATCH (g:Gene {symbol: $geneSymbol})
MATCH (g)-[:ENCODES]->(p:Protein)
MATCH (po:ProteinObservation)-[:MEASURES_PROTEIN]->(p)
MATCH (s:Sample)-[:HAS_PROTEIN_OBSERVATION]->(po)
OPTIONAL MATCH (out:ProcessedDataOutput)-[:CONTAINS_OBSERVATION]->(po)
RETURN s.sampleId,
       p.proteinId,
       p.symbol,
       po.abundanceValue,
       po.abundanceUnit,
       po.modificationType,
       po.quantificationMethod,
       po.sourceDataset,
       out.outputId
LIMIT 50;
```

### Example: find pathways associated with a gene and its encoded protein

```cypher
MATCH (g:Gene {symbol: $geneSymbol})
OPTIONAL MATCH (g)-[:PARTICIPATES_IN_PATHWAY]->(genePathway:Pathway)
OPTIONAL MATCH (g)-[:ENCODES]->(p:Protein)-[:PARTICIPATES_IN_PATHWAY]->(proteinPathway:Pathway)
OPTIONAL MATCH (proteinPathway)-[:PART_OF_PATHWAY]->(parentPathway:Pathway)
RETURN g.symbol,
       p.proteinId,
       collect(DISTINCT genePathway.name) AS genePathways,
       collect(DISTINCT proteinPathway.name) AS proteinPathways,
       collect(DISTINCT parentPathway.name) AS parentPathways
LIMIT 50;
```
