...
flowchart LR
  ...
  %% ------------------------------------------------------------------
  %% LifeSphere active schema overview
  %% Complete: all 67 relationship patterns from Section 5 are drawn.
  %% ------------------------------------------------------------------
  ...
  RegulatoryElement -->|LOCATED_IN_REGION| GenomicRegion[GenomicRegion]
  ...
  CpGSite[CpGSite] -->|MAPS_TO_GENE| Gene
  ...
  VariantObservation -->|OBSERVED_VARIANT| Variant
  ProteinObservation -->|MEASURES_PROTEIN| Protein
  MetaboliteObservation -->|MEASURES_METABOLITE| Metabolite[Metabolite]

  %% Methylation rule and classification
...
| Property | is_key | Data Type | Description | Example | Source / Origin |
| -------- | ------ | --------- | ----------- | ------- | --------------- |
| `cpgId` | Yes | String | Primary key for the CpG/probe feature in LifeSphere. This should be a platform-qualified identifier generated from the methylation platform code and the source CpG/probe ID using the pattern `{platformCode}:{sourceCpgId}` (`:`, LifeSphere's default id-minting separator — see docs/unique_ids.md). This prevents ambiguity when the same source probe ID or feature naming convention appears across methylation platforms or manifests. | `450k:cg00000029` | Derived from methylation platform metadata and manifest |
...
| `cpgId` | No | String | Mirror ID of the measured CpG/probe feature. This must match the platform-qualified `CpGSite.cpgId`, for example `{platformCode}:{sourceCpgId}`. Graph traversal should use `(:MethylationObservation)-[:MEASURES_CPG]->(:CpGSite)`. | `450k:cg00000029` | Derived from methylation matrix / manifest / platform metadata |
