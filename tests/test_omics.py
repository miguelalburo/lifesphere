...
def test_observation_nodes_carry_measurement_props(std_dir):
    ...
    assert "sampleId" not in row and "geneId" not in row and "subjectId" not in row
...
def test_variant_node_keeps_gene_annotation(std_dir):
    ...
    assert v["geneId"] == "ENSG001"    # gene_ensembl aliased → gene_id → geneId
...
def test_omics_edge_referential_integrity(std_dir):
    ...
    checks = [
        ...
        ("IS_WITHIN_GENE", "Variant", "Gene"),       # renamed from AFFECTS_GENE
    ...
    # OF_CELL_TYPE and CellType are retired; no such CSV should exist.
    assert not (std_dir / "edges" / "OF_CELL_TYPE.csv").exists()
    assert not (std_dir / "nodes" / "CellType.csv").exists()
...
def test_pseudobulk_folds_into_expression(std_dir):
    """Pseudobulk is an ExpressionObservation node with assay_type=pseudobulk;
    bulk rows share the node label but carry no cell-type edge."""
    expr = {r["id"]: r for r in _node(std_dir, "ExpressionObservation")}
